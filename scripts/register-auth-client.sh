...
echo ""
echo "Scope registered. OAuth clients are declared in config/registration.json"
echo "(auth.apiClient / auth.webClient sections) and picked up automatically by"
echo "andy-auth's SeedFromManifestsAsync on next startup. Schema:"
echo "  andy-service-template/docs/registration.schema.json"
echo ""
echo "For one-shot seeding against a running andy-auth instance, see the"
echo "register-service.sh bridge (tracked: andy-service-template#3)."
echo ""
echo "After manifest changes, restart andy-auth to re-seed:"
